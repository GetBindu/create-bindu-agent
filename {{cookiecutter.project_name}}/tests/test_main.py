...
async def test_handler_returns_response():
    ...
    with patch("{{cookiecutter.project_slug}}.main._initialized", True), \
...
async def test_handler_with_multiple_messages():
    ...
    with patch("{{cookiecutter.project_slug}}.main._initialized", True), \
         patch("{{cookiecutter.project_slug}}.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
